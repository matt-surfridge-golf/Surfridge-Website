...
SITE_URL = "https://surfridgegolf.com"

# Hand-written pages that belong in the sitemap; review pages are appended
# automatically from _reviews/. "" is the home page (index.html).
# /reviews/ is deliberately absent — it is a noindex stub that canonicalises
# to /reviews.html.
STATIC_PAGES = ["", "reviews.html", "store.html"]
...
def git_lastmod(relpath: str) -> str | None:
    """Date of the last commit to touch relpath (YYYY-MM-DD), or None.

    Used for the hand-written pages, which carry no date of their own. Review
    pages use their JSON 'date' instead, so the sitemap stays reproducible even
...
def build_sitemap(reviews: list[dict]) -> str:
    entries: list[tuple[str, str | None]] = [
        (f"{SITE_URL}/{page}", git_lastmod(page or "index.html"))
        for page in STATIC_PAGES
    ]
    entries += [
        (f"{SITE_URL}/reviews/{r['slug']}/", r["date"]) for r in reviews
    ]
    ...
    return (
        ...
        "<!-- Generated by tools/build_reviews.py — do not edit by hand. -->\n"
...
def update_sitemap(reviews: list[dict], check: bool) -> bool:
    new = build_sitemap(reviews)
    existing = SITEMAP.read_text(encoding="utf-8") if SITEMAP.exists() else None
    if existing == new:
        return False
    if not check:
...
def main() -> None:
    ...
    if update_sitemap(reviews, args.check):
